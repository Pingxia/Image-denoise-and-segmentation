...
if __name__== "__main__":
	if (len(sys.argv) != 4):
		print ("Usage: python em.py 'CONVERGENCE_THRESHOLD' 'input file path' 'output folder'")
		print ("Eg: python em.py 0.01 ../a2/cow.txt ./cow/")
		print ("Result images will be generated in output folder")
		exit(0)

	CONVERGENCE_THRESHOLD = float(sys.argv[1])
	input_path = sys.argv[2]
	output_path = sys.argv[3]

	data, image = read_data(input_path, True)
	...
	# save result images
	imageplt.imsave(output_path + 'mask.png', mask)
	cv2.imwrite(output_path + 'foreground.png', (cv2.cvtColor(foreground, cv2.COLOR_Lab2BGR) * 255).astype(np.uint8))
	cv2.imwrite(output_path + 'background.png', (cv2.cvtColor(background, cv2.COLOR_Lab2BGR) * 255).astype(np.uint8))
